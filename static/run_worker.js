// most of the code is from this repo: [https://github.com/frto027/IsaacDisassemblyRate]
...
// heyn's note: what even is this lol
let btree_nodes = {
...
let item_config_data = {
    ...
    256:{quality:1},
    257:{quality:2},
    258:{quality:1,achievement_id:105},
    259:{quality:3},
    ...
    476:{quality:3,achievement_id:296},
    ...
    651:{quality:3,achievement_id:425},
    652:{quality:1},
...
function* getRandomPool(n) {
    while (true) {
        ...
        let r = possible_options[Math.floor(Math.random() * possible_options.length)];
        if (r == 23) {
            pos_opt.splice(pos_opt.indexOf(8), 1);
            pos_opt.splice(pos_opt.indexOf(15), 1);
            pos_opt.splice(pos_opt.indexOf(12), 1);
            pos_opt.splice(pos_opt.indexOf(1), 1);
        } else {
            pos_opt.splice(pos_opt.indexOf(23), 1);
        }
        a = new Array()
        for (let i = 0; i < n; i++) {
            a[i] = pos_opt[Math.floor(Math.random() * pos_opt.length)];
        }
        a = a.sort()
        delete pos_opt
...
let chances = {
    ...
    21: 3, // card
    22: 3, // pill
    23: 1, // rune shard //custom
}
for (const [key,value] of Object.entries(chances)) {
    for (let i = 0; i < value; i++) {
        possible_options.push(parseInt(key))
    }
}
let found = 0
let tries = 0
